...
def get_args_parser():
    ...
    parser.add_argument('--data-use-percent', default=1.0, type=float,
                    help='Percentage of the dataset to use (value between 0 and 1)')
...
def split_dataset(dataset_path, val_split, transform, data_use_percent=1.0, seed=None):
    ...
    samples_per_class = int(min(len(indices) * data_use_percent for indices in class_indices))

    # Collect equal number of samples from each class
    selected_indices = []
    ...
    split = int(len(selected_indices) * (1 - val_split))
...
def get_data_loaders(args):
    # Define the normalization for the dataset
    ...
    # Load STL10 dataset
    # train_dataset = Food101(root=args.data, split='train', transform=train_transform, download=True)
    val_dataset_full = datasets.STL10(root=args.data, split='test', transform=val_transform, download=True)

    num_samples = len(val_dataset_full)
    subset_size = num_samples // 2  # Integer division for 50%
    ...
    #     args.val_split, 
    #     train_transform,
    #     data_use_percent=args.data_use_percent
    # )



    train_dataset = datasets.ImageFolder(
        os.path.join(args.data, 'STL10SyntheticImages'), train_transform)
    # val_dataset = datasets.ImageFolder(
    #     os.path.join(args.data, 'imagesupdate'), val_transform)
...
def validate(val_loader, model, linear_classifiers, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            for classifier_key, classifier_output in outputs.items():
                _, preds = torch.max(classifier_output, 1)
    ...
    for i in range(num_classes):
      print(f"Accuracy of class {i}: {per_class_accuracies[i].item() * 100:.2f}%")
    return {'acc1': top1.avg, 'acc5': top5.avg, 'loss': losses.avg}, all_top1, all_top5, all_losses
